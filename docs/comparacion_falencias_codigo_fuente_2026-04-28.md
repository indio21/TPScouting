...
Resultado validado tras arquitectura fase 2 staff/players: `51 passed`, cobertura total `77%`, con 4 warnings conocidos de scikit-learn por fixtures con columnas all-NaN en tests.
...
- Falencias que siguen abiertas como deuda real: dashboard/comparadores/settings todavia en `app.py`, rendimiento del dashboard a gran escala, tooling dev opcional y decisiones productivas de despliegue/concurrencia.
...
| CALIDAD-01 app.py monolitico | Fase 1 de arquitectura aplicada: cache, seguridad liviana, mantenimiento operativo y runtime ML pasaron a `services/` y `ml/`. Fase 2 avanzo con `auth`, `staff` y `players` en `routes/`, conservando aliases legacy. | Mejorado fuerte, todavia parcial | Faltan blueprints `dashboard`, `compare` y `settings`; helpers compartidos siguen en `app.py`. |
...
| TEST-01 cobertura baja | CI y local miden cobertura con `pytest-cov`; suite actual con cobertura `51 passed`, total `77%`. | Corregido/medido | Subir cobertura de `app.py` seria mejora incremental, no bloqueo. |
...
- En `reformas-complejas` ya avanzo el refactor de arquitectura con servicios y runtime ML extraidos, y con rutas de `auth`, `staff` y `players` movidas a blueprints.
...
3. Refactor de arquitectura fase 2: continuar rutas a blueprints por dominio. `auth`, `staff` y `players` ya fueron movidos; queda `compare`, `dashboard` y `settings`.
