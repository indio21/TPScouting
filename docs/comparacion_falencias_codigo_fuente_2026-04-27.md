...
Resultado: `42 passed`, cobertura total reportada `75%`, con 4 warnings conocidos de scikit-learn por fixtures con columnas all-NaN en tests.
...
| TEST-01 cobertura muy baja | Medido formalmente | 90% | La suite tiene `42` tests y CI mide cobertura con `pytest-cov`; la corrida local reporto `75%` total. |
...
- Auditoria CSRF ruta por ruta y tests para cada POST mutante critico.
